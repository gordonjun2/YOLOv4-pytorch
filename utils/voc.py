...
def parse_mstar_annotation(
    ...
    classes = cfg.MSTAR_DATA["CLASSES"]
...
if __name__ == "__main__":
    ...
    if dataset_name != 'MSTAR':
        ...
        # train_set : MSTAR_detection
        train_data_path_mstar = os.path.join(
            cfg.DATA_PATH, "MSTAR_detection", "VOCdevkit2007", "VOC2007"
        )
        train_annotation_path_mstar = os.path.join("../data", "train_annotation_mstar.txt")
        if os.path.exists(train_annotation_path_mstar):
            os.remove(train_annotation_path_mstar)

        # val_set   : MSTAR_detection
        test_data_path_mstar = os.path.join(
            cfg.DATA_PATH, "MSTAR_detection", "VOCdevkit2007", "VOC2007"
        )
        test_annotation_path_mstar = os.path.join("../data", "test_annotation_mstar.txt")
        if os.path.exists(test_annotation_path_mstar):
            os.remove(test_annotation_path_mstar)

        len_train = parse_mstar_annotation(
            train_data_path_mstar,
            "trainval",
            train_annotation_path_mstar,
            use_difficult_bbox=False,
        )

        len_test = parse_mstar_annotation(
            test_data_path_mstar,
            "test",
            test_annotation_path_mstar,
            use_difficult_bbox=False,
        )
